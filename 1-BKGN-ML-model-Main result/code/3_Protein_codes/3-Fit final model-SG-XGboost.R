...
ranger_exp = explain_mlr3(learner_dalex,
                          ...
                          colorize = F)
rf_pd = model_profile(ranger_exp,
                      variables = c("X36"))$agr_profiles #Cr_2 in SR-2 sample
rf_pd = model_profile(ranger_exp,
                      variables = c("pred.1.Serum2", "pred.1.Hair", "pred.1.FF", "pred.1.Serum1"))$agr_profiles #predctive value for Serum2
# plot-----------------
rf_pd  %>% 
  as_tibble() %>% 
  left_join(eSet$FN$xg$Voca %>% dplyr::select(VarLabel, VarName), by = c("_vname_"="VarName"))  %>% 
...
plotdata %>% 
  ...
  geom_smooth(se = T) + 
  # scale_y_continuous(
  #   limits = c(0.445, 0.495), breaks = seq(0.45, 0.49, by = 0.01),
  #   expand = expansion(add = c(0.00, 0.00))) + 
  scale_x_continuous(expand = expansion(add = c(0.02, 0.02))) + 
  facet_wrap(~VarName, ncol = 5, scales = "free") + 
  theme_bw()+
  theme(
...
#save
ggsave(str_c(work.dir,"/output","/FigS3-[Cr_2].jpg"),
