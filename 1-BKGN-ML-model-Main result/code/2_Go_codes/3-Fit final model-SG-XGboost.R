...
inpath = str_c(str_c(work.dir,"/input/2-GO.Addition"))
outpath = str_c(str_c(work.dir,"/output/2-GO.Addition"))
load(str_c(outpath, "/eSet_Feature_Select.GOs.RData"))
eSet$FinalFeatures
...
Base.model = function(i, Data, Voca){
  ...
  if(nrow(eSet$FinalFeatures[[i]])==1){
    eSet$FinalFeatures[[i]] %>% 
      mutate(importance_perct = 1,
             importance_perct_cum = 1) %>%
  ...
  }else if(nrow(eSet$FinalFeatures[[i]])>1){
...
# Extract exposures of covariates------------------
...
eSet$FN$xg$Voca %>% 
  ...
  dplyr::mutate(VarLabel = str_replace(VarLabel, "GO", "GOs1")) 
  , by = "VarLabel") %>% 
  ...
  dplyr::select(VarLabel, OmicGroup) %>% 
  dplyr::mutate(VarLabel = str_replace(VarLabel, "GO", "GOs2"))
  , by = "VarLabel") %>%
  left_join(eSet$H1$Voca %>% 
  dplyr::select(VarLabel, OmicGroup) %>% 
  dplyr::mutate(VarLabel = str_replace(VarLabel, "GO", "GOh1"))
  , by = "VarLabel") %>% 
  left_join(eSet$FF$Voca %>% 
  dplyr::select(VarLabel, OmicGroup) %>% 
  dplyr::mutate(VarLabel = str_replace(VarLabel, "GO", "GOff"))
  , by = "VarLabel") -> test
test  %>% 
  print(n=nrow(.))
# tidy
...
# 2.2.3 Train model and rank feature importance -----------------
...
df.train %>%
  dplyr::select(Y, any_of(eSet$FinalFeatures[["FinalModel"]]$Features))  -> df.dalex
# learner
# -xgboost(needs to be tuned)-
learner_dalex = lrn(str_c("classif",".xgboost"),
                 predict_sets = c("train", "test"),
                 predict_type = "prob")
learner_dalex$param_set$values$eta = 0.1


df.dalex %>% 
  as_task_classif(target = VarY) -> task.dalex
learner_dalex$train(task.dalex)


df.dalex$Y = as.numeric(df.dalex$Y)
set.seed(222)

ranger_exp = explain_mlr3(learner_dalex,
                          data = df.dalex[, -1],
                          y = df.dalex$Y,
                          label = "XGBoost",
                          colorize = F)
rf_pd = model_profile(ranger_exp,
                      variables = c("X64"))$agr_profiles #Se_2 in SR-2 sample
...
# plot
plotdata %>% 
  dplyr::filter(VarLabel == "Se_2") %>% 
  ggplot(aes(x = x, y = yhat)) +
  geom_smooth(se = T) + 
  scale_y_continuous(expand = expansion(add = c(0.02, 0.02))) + 
  scale_x_continuous(expand = expansion(add = c(0.02, 0.02))) + 
...
#save
ggsave(str_c(work.dir,"/output","/FigS3-[Se_2].jpg"),
