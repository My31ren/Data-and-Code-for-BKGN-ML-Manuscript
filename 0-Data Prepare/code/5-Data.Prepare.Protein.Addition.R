...
# 2-Combine Protein information ------------------------
# load protein matrix 
load(str_c(outpath,"/EXP_matrix_full.rdata"))

index_matrix2$name
index_s1 = str_c(index_matrix2$name, "_1")
index_s2 = str_c(index_matrix2$name, "_2")
index_ff = str_c(index_matrix2$name, "_F")
index_hr = str_c(index_matrix2$name, "_H1")
...
# 6-export ------------------------
if(!file.exists(str_c(outpath,"/3-Protein.Addition"))){
  dir.create(str_c(outpath,"/3-Protein.Addition"))
}
tmp.s1 %>%
  writexl::write_xlsx(str_c(outpath,"/3-Protein.Addition","/eSetdata_serum1.xlsx"))
tmp.s2 %>%
  writexl::write_xlsx(str_c(outpath,"/3-Protein.Addition","/eSetdata_serum2.xlsx"))
tmp.ff %>%
  writexl::write_xlsx(str_c(outpath,"/3-Protein.Addition","/eSetdata_ff.xlsx"))
tmp.hr %>%
  writexl::write_xlsx(str_c(outpath,"/3-Protein.Addition","/eSetdata_hr.xlsx"))
