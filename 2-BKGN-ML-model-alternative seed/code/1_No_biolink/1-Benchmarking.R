...
ggsave(str_c(outpath, "/BenchMarking.NoBiolink-Test.dataset.png"), width = 12, height = 7)
...
plotdata %>% 
  ...
  scale_fill_manual(values = c("#2F4F4F","#FFB366","#0099B4","#ED0000","#925E9F","#42b540"), name = "Algorithms") +  # Custom fill colors
  scale_color_manual(values = c("#2F4F4F","#FFB366","#0099B4","#ED0000","#925E9F","#42b540"), name = "Algorithms")  # Custom line colors
ggsave(str_c(outpath, "/BenchMarking.NoBiolink-Train.dataset.png"), width = 12, height = 7)


# #using ggplot2
# plotdata %>% 
#   tidyr::pivot_longer(cols = acc_train:npv_test,
#                       names_to = "msrs", 
#                       values_to = "value") -> plt2
...
# }
# plt2 %>% 
#   dplyr::filter(OmicGroup == "Baseline") %>% 
#   dplyr::filter(str_detect(msrs, "_test")) %>% 
#   arrange(OmicGroup, learner_id, msrs)  %>% 
#   group_by(OmicGroup, learner_id) %>% 
#   group_modify( ~ rbind(.x, head(.x, 1))) %>% 
#   ggplot(aes(x = msrs, y = value, group = learner_id)) + 
#   geom_line(aes(color = factor(learner_id))) + 
#   geom_point(aes(color = factor(learner_id))) + 
...
# Radar.plt = function(i){
# plotdata %>%
#   dplyr::select(OmicGroup, learner_id, 
#                 acc_test, precision_test, sensitivity_test, specificity_test, npv_test) %>%
#   set_names(c("OmicGroup", "learner_id", 
#                 "ACC", "PPV", "TPR", "TNR", "NPV")) %>%  
#   dplyr::filter(OmicGroup==Omics[[i]]) %>% 
#   dplyr::select(-OmicGroup) %>% 
#   ggradar(
#     base.size = 15,
#     axis.label.size = 3,
#     values.radar = c("0", "0.4", "0.8"),
#     grid.min = 0, grid.mid = 0.4, grid.max = 0.85,
#     # Polygons
#     group.line.width = 1, 
#     group.point.size = 3,
#     # group.colours = eSet$ModelStat[[1]] %>% 
#     # dplyr::select(learner_id)  %>% as.matrix() %>% as.vector(),
...
# # 4----plot for Figure 3A----
# load(str_c(outpath, "/eSet_BenchMark.RData"))
# tmp = tibble(learners = character(),
#              RsmpMethod = character(),
#              acc_train = numeric(),
#              acc_test_int = numeric(),
#              group = character())
# f1 = function(i){
#   tmp %>% 
#     rbind(eSet$ModelStat[[i]] %>% 
#             as_tibble() %>% 
#             dplyr::mutate(group =list.names(eSet$ModelStat[i]))) -> tmp
#   return(tmp)
# }
# #对于No-Biolink
# purrr::map_dfr(1:length(eSet$ModelStat), f1) -> plotdata
# plotdata %>% 
#   tidyr::pivot_longer(cols = c("acc_train", "acc_test_int"),
#                       names_to = "facet",
#                       values_to = "acc")  -> plotdata

# library(ggsci)

# plotdata %>% 
#   dplyr::filter(facet == "acc_test_int") %>% 
#   ggplot(aes(x=learners,y=acc,fill=group))+
#     geom_bar(stat = 'identity',#柱状图位置并排:
#              position = 'dodge', 
...
# ggsave(str_c(outpath, "/BenchMarking.NoBiolink.png"), width = 12, height = 7)
