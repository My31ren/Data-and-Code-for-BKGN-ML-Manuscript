...
if(!file.exists(str_c(work.dir,"/input/2-GO.Addition"))){
  dir.create(str_c(work.dir,"/input/2-GO.Addition"))
}
if(!file.exists(str_c(work.dir,"/output/2-GO.Addition"))){
  dir.create(str_c(work.dir,"/output/2-GO.Addition"))
}
...
OmicGrp = "Serum1"
eSet$S1$Voca %>%
    dplyr::filter(OmicGroup == OmicGrp | OmicGroup == "GO1") %>%
    dplyr::select(VarName) %>%
    as.matrix() %>% as.vector()-> Xlist[[3]] 
OmicGrp = "Serum2"
eSet$S2$Voca %>%
    dplyr::filter(OmicGroup == OmicGrp| OmicGroup == "GO2") %>%
    dplyr::select(VarName) %>%
    as.matrix() %>% as.vector()-> Xlist[[4]] 
OmicGrp = "FF"
eSet$FF$Voca %>%
    dplyr::filter(OmicGroup == OmicGrp| OmicGroup == "GOff") %>%
    dplyr::select(VarName) %>%
    as.matrix() %>% as.vector()-> Xlist[[5]] 
OmicGrp = "Hair"
eSet$H1$Voca %>%
    dplyr::filter(OmicGroup == OmicGrp| OmicGroup == "GOhr") %>%
    dplyr::select(VarName) %>%
...
#save rData
save.image(str_c(outpath, "/eSet_BenchMark.GOs.RData"))

# # 5----plot-------------------
load(str_c(outpath, "/eSet_BenchMark.GOs.RData"))
eSet$ModelStat
...
ggsave(str_c(outpath, "/BenchMarking.GO.Addition-Test.dataset.png"), width = 12, height = 7)
...
plotdata %>% 
  ...
  scale_fill_manual(values = c("#2F4F4F","#FFB366","#0099B4","#ED0000","#925E9F","#42b540"), name = "Algorithms") +  # Custom fill colors
  scale_color_manual(values = c("#2F4F4F","#FFB366","#0099B4","#ED0000","#925E9F","#42b540"), name = "Algorithms")  # Custom line colors
ggsave(str_c(outpath, "/BenchMarking.GO.Addition-Train.dataset.png"), width = 12, height = 7)



# ggsave(str_c(outpath, "/BenchMarking.GO.Addition.png"), width = 12, height = 5)
